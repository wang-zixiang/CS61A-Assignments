...
class HungryAnt(Ant):
    ...
    food_cost = 4
    chewing_turns = 3
    # turn_to_chew = 0 #记录距离下一次咀嚼，还要几个回合
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_waterproof:
            insect.reduce_health(self.palce.health)
        # END Problem 10
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self, health=1):
        ...
        if QueenAnt.has_queen:
            ...
            QueenAnt.has_queen = True # 标记女王已诞生
    ...
    def action(self, gamestate):
        ...
        while p is not None:
            ...
            if p.ant is not None and p.ant is not self:
                p.ant.double()
                #如果这个是容器
                if p.ant.is_container and p.ant.ant_contained is not None and p.ant.ant_contained is not self:
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0 and not self.is_impostor:
            ants_lose()
        # END Problem 12
